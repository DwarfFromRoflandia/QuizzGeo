...
//видео, которые помогли разобраться, как реализовать свайпы панелей с уровнями: https://www.youtube.com/watch?v=tCr_i5CVv_w&t=4s и https://www.youtube.com/watch?v=rjFgThTjLso


//справа от нашего MonoBehaviour, мы хотим унаследоваться ещё от двух классов: IDragHandler и IEndDragHandler
public class PageSwaperLevelSelected : MonoBehaviour, IDragHandler, IEndDragHandler //эти два класса позволяют использовать методы, которые нужны для определение того, когда перетаскивание панели(свайп) начинается и когда оно заканчивается
{
    ...
    public void OnEndDrag(PointerEventData data)
    {
        ...
        if (Mathf.Abs(percentage) >= precentThreshold)//если наше процентное значение отрицательное, то мы хотим настроить новое местоположение для перемещения нашей панели
        {
            ...
            for (int i = 0; i < _Pointers.Length; i++)//ДОБАВЛЕНИЕ УКАЗАТЕЛЕЙ ОПРЕДЕЛЁННОЙ СТРАНИЦЕ НА КОТОРОЙ НАХОДИТСЯ ИГРОК
            {
                if (currentPage == 1)
                {
